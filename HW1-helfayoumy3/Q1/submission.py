...
class Graph:
    ...
    def add_node(self, id: str, name: str)->None:
        ...
        """
        tupObject = (id, name)
        if tupObject not in self.nodes:
            self.nodes.append(tupObject)
            self.totalNodes += 1
    ...
    def add_edge(self, source: str, target: str)->None:
        ...
        """
        tupObject = (source, target)
        tupObject2 = (target, source)
        if tupObject not in self.edges and tupObject2 not in self.edges:
            self.edges.append(tupObject)
            self.totalEdges += 1
    ...
    def max_degree_nodes(self)->dict:
        ...
        maxEdges = max(dictionary.items(), key=lambda x: x[1])

        for key, value in dictionary.items():
            if value == maxEdges[1]:
                dictHighest[key] = value
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        """
        h1 = http.client.HTTPConnection('api.themoviedb.org')
        h1.request('GET', '/3/movie/' + str(movie_id) + '/credits?api_key=' + str(self.api_key)) 
        h2 = json.loads(h1.getresponse().read())
        
        if exclude_ids is None:
            exclude_ids = list()

        listCastMembers = list()
        for i in h2['cast']:
            if i['id'] not in exclude_ids:
                if limit:
                    if i['order'] < limit:
                        castDict = dict()
                        castDict['id'] = i['id']
                        castDict['character'] = i['character']
                        castDict['name'] = i['name'].replace(',', '')
                        castDict['credit_id'] = i['credit_id']
                        listCastMembers.append(castDict)
                else:
                    castDict = dict()
                    castDict['id'] = i['id']
                    castDict['character'] = i['character']
                    castDict['name'] = i['name']
                    castDict['credit_id'] = i['credit_id']
                    listCastMembers.append(castDict)
        
        return listCastMembers
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        """
        h1 = http.client.HTTPConnection('api.themoviedb.org')
        h1.request('GET', '/3/person/' + person_id + '/movie_credits?api_key=' + self.api_key + '&language=en-US')
        h2 = json.loads(h1.getresponse().read())
        # h3 = h2.read()
        # h4 = json.loads(h3)
        listMovies = list()
        for i in h2['cast']:
            if i['vote_average'] >= vote_avg_threshold:
                movieDict = dict()
                movieDict['id'] = i['id']
                movieDict['title'] = i['title']
                movieDict['vote_avg'] = i['vote_average']
                listMovies.append(movieDict)
...
# snapshot.
#
# Build your co-actor graph on the actress 'ds' w/ person_id 5064.
...
def return_argo_lite_snapshot()->str:
    ...
    """
    return 'https://poloclub.github.io/argo-graph-lite/#83ad7a75-3150-4d31-b5b1-6aac2da5e995'
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='b82cb6ad7f66baad463456c0b3b4cd94')
    ...
    movieCred = tmdb_api_utils.get_movie_credits_for_person(str(currId), 8.0)
    for i in movieCred:
        retInfo = tmdb_api_utils.get_movie_cast(str(i['id']), 3, excludeIds)
        for x in retInfo:
            graph.add_node(str(x['id']), x['name'])
            graph.add_edge(str(currId), str(x['id']))
            oof.append(x['id'])
